...
<body>
	...
	아이디 : <%= user.getRoot_id() %> / 이름 : <%= user.getRoot_name() %>회원님 반갑습니다.
	<br> 고유번호 :	<%= user.getRoot_idn() %> / 이메일 : <%= user.getRoot_email() %> 입니다.
	<br>
	<a href="item_list.do">분류항목 목록</a>&nbsp;&nbsp;&nbsp; <a href="item_write.do">분류항목 입력</a>
	<br>
	<a href="expense_all_list.do">지출 목록</a>&nbsp;&nbsp;&nbsp; <a href="expense_write.do">지출 입력</a>
	<br>
	<a href="asset_list.do">자산 목록</a>&nbsp;&nbsp;&nbsp; <a href="asset_write.do">자산 입력</a>
	<br>
	<a href="loan_list.do">대출 목록</a>&nbsp;&nbsp;&nbsp; <a href="loan_write.do">대출 입력</a>
	<br>
	<a href="card_list.do">카드 목록</a>&nbsp;&nbsp;&nbsp; <a href="card_write.do">카드 입력</a>
	<br>
	...
	<div class="frame">
		...
		<table class="table table-bordered text-center">
			<ul id="asset_top" style="padding:0; border:0; margin:0;">
				...
				<li id="asset_add"><b><input type="button" class="white_btn" id="add_row" value="자산항목추가" style="padding: 0; height: 30px;"></b></li>
			</ul>
			<colgroup>
				<col width="200px" />  <col width="200px" />  <col width="80px" />  <col width="100px" />  <col width="120px" />  	<col width="100px" /> <col width="100px" />
				<col width="120px" />
				<col width="80px" />
			</colgroup>
			<thead><tr>
				<th style="line-height: 40px;">계좌명</th> <th style="line-height: 40px;">용도</th> <th style="line-height: 40px;">자산유형</th>  <th style="line-height: 40px;">시작일자</th>  <th style="line-height: 40px;">거래은행</th>  <th style="line-height: 40px;">초기 금액</th>  <th style="line-height: 40px;">현재 잔액</th>  <th style="line-height: 40px;">사용함</th>  <th style="line-height: 40px;">삭제</th>

			</tr></thead>
			<tbody id="asset_table">
			<c:forEach items="${ assetList }" var="asset">
				<tr class="asset_list">
					<td class="text" style="padding:0;"><input type="text" class="modify" name="asset_name" value="${ asset.asset_name }" size="20" style="line-height: 40px; width:200px;"></td>
					<td class="text" style="padding:0;"><input type="text" class="modify" name="asset_purpose" value="${ asset.asset_purpose }" size="20" style="line-height: 40px; width:200px;"></td>
							...
							<c:forEach items="${ trade }" var="trade">
								<c:choose>
									<c:when test="${ trade.trade_name == asset.trade_name }">
										<option value="${ trade.trade_code }" selected="selected"><c:out value="${ trade.trade_name }"/></option>
									</c:when>
									<c:when test="${ trade.trade_name != asset.trade_name }">
					...
					<td class="date" style="padding:0;"><input type="date" name="asset_date" value="${ asset.asset_date }" size="20" style="line-height: 40px;"></td>
					<td class="text" style="padding:0;">
							...
							<c:forEach items="${ bank }" var="bank">
								<c:choose>
									<c:when test="${ bank.bank_name == asset.bank_name }">
										<option value="${ bank.bank_code }" selected="selected"><c:out value="${ bank.bank_name }"/></option>
									</c:when>
									<c:when test="${ bank.bank_name != asset.bank_name }">
					...
					<td class="amount" style="padding:0;"><b><input type="text" id="basic_amount" name="basic_amount" numberOnly value="${ asset.basic_amount }" size="10" style="text-align: right; padding-right: 5px; line-height: 40px;"></b></td>
					...
					<td class="text" style="line-height: 45px; padding:0;"><input type="checkbox" name="check" value="${ asset.asset_code }" checked="checked">사용중</td>
					<td class="text" style="line-height: 45px; padding:0;"><input type="button" class="white_btn" id="row_remove" value="삭제" size="50"></td>
				</tr>
			</c:forEach>
			</tbody>
		</table>
		<div style="width:100px; margin: 0 auto;">
			...
			<input type="button" name="return_list" value="취소">
		</div>
	</div>
	</form>
...
$(function() {
	$('#add_row').click(
		function() {
			...
			var rowItem = "<tr>"
			rowItem += '<td class="text" style="padding:0;"><input type="text" class="modify" name="asset_name" size="20" style="line-height: 40px;"></td>'
			rowItem += '<td class="text" style="padding:0;"><input type="text" class="modify" name="asset_purpose" size="20" style="line-height: 40px;"></td>'
			rowItem += '<td class="text" style="padding:0;"><select name="trade_code" ><option value=""></option></select></td>'
			...
			rowItem += '<td class="text" style="padding:0;"><select name="bank_code" ><option value=""></option></select></td>'
			rowItem += '<td class="text" style="padding:0;"><b><input type="text" class="modify" name="basic_amount" numberOnly id="basic_amount" size="10" style="text-align: right; padding-right: 5px;"></b></td>'
