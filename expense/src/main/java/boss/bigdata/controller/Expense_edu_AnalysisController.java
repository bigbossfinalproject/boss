...
@Controller
public class Expense_edu_AnalysisController {

	@Autowired
	IncomeAnalysisDao dao;
	
	
	@RequestMapping(value="/edu_expense_age.do")
	public ModelAndView edu_expense_age(HttpServletRequest request, HttpServletResponse response){
		...
		try {
            ...
            connection.eval("edu_expense <- bigdata %>% "
              + " filter(!is.na(expense_total)) %>% "
              + " group_by(root_edu, root_agegen) %>% "
              + " summarise(mean_expense = mean(expense_total))");
            connection.eval("a <- ggplot(data=edu_expense, aes(x=root_edu, y=mean_expense, fill=root_agegen))"
		...
		mav.setViewName("/bigdata/total_analysis/edu_expense/edu_expense_age_result");
		return mav;
	}

	@RequestMapping(value="/edu_expense_marry.do")
	public ModelAndView edu_expense_marry(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_expense <- bigdata %>% "
              ...
              + " summarise(mean_expense = mean(expense_total))");
            connection.eval("a <- ggplot(data=edu_expense, aes(x=root_edu, y=mean_expense, fill=root_marry))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소비\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_expense_marry.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_expense/edu_expense_marry_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_expense_job.do")
	public ModelAndView edu_expense_job(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_expense <- bigdata %>% "
              ...
              + " summarise(mean_expense = mean(expense_total))");
            connection.eval("a <- ggplot(data=edu_expense, aes(x=root_edu, y=mean_expense, fill=root_job))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소비\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_expense_job.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_expense/edu_expense_job_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_expense_gender.do")
	public ModelAndView edu_expense_gender(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
		...
		mav.setViewName("/bigdata/total_analysis/edu_expense/edu_expense_gender_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_expense_loc.do")
	public ModelAndView edu_expense_loc(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_expense <- bigdata %>% "
              ...
              + " summarise(mean_expense = mean(expense_total))");
            connection.eval("a <- ggplot(data=edu_expense, aes(x=root_edu, y=mean_expense, fill=root_loc))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소비\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_expense_loc.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_expense/edu_expense_loc_result");
		return mav;
	}
