...
@Controller
public class Income_edu_AnalysisController {

	@Autowired
	IncomeAnalysisDao dao;
	
	@RequestMapping(value="/edu_income_age.do")
	public ModelAndView edu_income_age(HttpServletRequest request, HttpServletResponse response){
		...
		try {
            ...
            connection.eval("edu_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_edu, root_agegen) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=edu_income, aes(x=root_edu, y=mean_income, fill=root_agegen))"
            ...
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_income_age.html\")");
		...
		mav.setViewName("/bigdata/total_analysis/edu_income/edu_income_age_result");
		return mav;
	}

	@RequestMapping(value="/edu_income_marry.do")
	public ModelAndView edu_income_marry(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_edu, root_marry) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=edu_income, aes(x=root_edu, y=mean_income, fill=root_marry))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_income_marry.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_income/edu_income_marry_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_income_job.do")
	public ModelAndView edu_income_job(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_edu, root_job) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=edu_income, aes(x=root_edu, y=mean_income, fill=root_job))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_income_job.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_income/edu_income_job_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_income_gender.do")
	public ModelAndView edu_income_gender(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
		...
		mav.setViewName("/bigdata/total_analysis/edu_income/edu_income_gender_result");
		return mav;
	}
	
	@RequestMapping(value="/edu_income_loc.do")
	public ModelAndView edu_income_loc(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("edu_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_loc, root_edu) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=edu_income, aes(x=root_edu, y=mean_income, fill=root_loc))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/edu_income_loc.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/edu_income/edu_income_loc_result");
		return mav;
	}
