...
@Controller
public class Income_job_AnalysisController {

	@Autowired
	IncomeAnalysisDao dao;
	
	@RequestMapping(value="/job_income.do", method=RequestMethod.GET)
	public ModelAndView income_age_form(HttpServletRequest request, HttpServletResponse response){
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_form");
		return mav;
	}
	
	@RequestMapping(value="/job_income_age.do")
	public ModelAndView job_income_age(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		...
		try {
            ...
            connection.eval("age_income <- bigdata %>% "
              ...
              + " group_by(root_job, agegen) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=age_income, aes(x=root_job, y=mean_income, fill=agegen))"
            ...
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/job_income_age.html\")");
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_age_result");
		return mav;
	}

	@RequestMapping(value="/job_income_marry.do")
	public ModelAndView job_income_marry(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("age_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_job, root_marry) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=age_income, aes(x=root_job, y=mean_income, fill=root_marry))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/job_income_marry.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_marry_result");
		return mav;
	}

	@RequestMapping(value="/job_income_gender.do")
	public ModelAndView job_income_gender(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("age_income <- bigdata %>% "
              ...
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=age_income, aes(x=root_job, y=mean_income, fill=root_gender))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/job_income_gender.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_gender_result");
		return mav;
	}

	@RequestMapping(value="/job_income_edu.do")
	public ModelAndView job_income_edu(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("age_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_job, root_edu) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=age_income, aes(x=root_job, y=mean_income, fill=root_edu))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/job_income_edu.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_edu_result");
		return mav;
	}
	
	@RequestMapping(value="/job_income_loc.do")
	public ModelAndView job_income_loc(HttpServletRequest request, HttpServletResponse response){
		response.setCharacterEncoding("UTF-8");
		ModelAndView mav = new ModelAndView();
		RConnection connection = null;
		try {
            ...
            connection.eval("age_income <- bigdata %>% "
              + " filter(!is.na(income_amount)) %>% "
              + " group_by(root_job, root_loc) %>% "
              + " summarise(mean_income = mean(income_amount))");
            connection.eval("a <- ggplot(data=age_income, aes(x=root_job, y=mean_income, fill=root_loc))"
              + "+geom_col(position=\"dodge\")"
              + "+theme(legend.title = element_blank() ,legend.background = element_rect(fill=\"gainsboro\", color = \"black\")"		
              + "+ggtitle(\"분석 그래프\")"
              + "+xlab(\"성별\")"
              + "+ylab(\"소득\")");
            connection.eval("p <- ggplotly(a)");
            connection.eval("saveWidget(p, file=\"C:/workspaces/finalproject/boss/src/main/webapp/resources/bigdata_html/job_income_loc.html\")");
            connection.eval("dev.off()");
        } catch (Exception e) {
            // TODO: handle exception
            System.out.println(e);
		...
		mav.setViewName("/bigdata/total_analysis/job_income/job_income_loc_result");
		return mav;
	}
